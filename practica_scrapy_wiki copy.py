...
# Palabras más frecuentes en latín
df_palabras_latin = pd.DataFrame(frecuencia_palabras_latin.most_common(10), columns=["Palabra_Latín", "Frecuencia_Latín"])

# Verbos más frecuentes en latín
df_verbos_latin = pd.DataFrame(frecuencia_verbos_latin.most_common(10), columns=["Verbo_Latín", "Frecuencia_Verbo_Latín"])

# Palabras más frecuentes en inglés
df_palabras_english = pd.DataFrame(frecuencia_palabras_english.most_common(10), columns=["Palabra_Inglés", "Frecuencia_Inglés"])

# Verbos más frecuentes en inglés
df_verbos_english = pd.DataFrame(frecuencia_verbos_english.most_common(10), columns=["Verbo_Inglés", "Frecuencia_Verbo_Inglés"])


df_frecuencias = pd.concat([df_palabras_latin, df_verbos_latin, df_palabras_english, df_verbos_english], axis=1)

archivo_frecuencias = "frecuencias_latín_inglés.xlsx"
df_frecuencias.to_excel(archivo_frecuencias, index=False)
